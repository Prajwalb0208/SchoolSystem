...
function setup() {
	...
	if (!player || player.length === 0) {
		player = [];
		for (var init = 0; init <= 8; init++) {
			player[init] = new Player("", "");
			player[init].index = init;
		}
		player[1].human = true;
	...
	for (var i = 1; i <= pcount; i++) {
		...
		if (!p) {
			p = new Player("", "");
			p.index = playerArray[i - 1];
			player[playerArray[i - 1]] = p;
		}

		var colorEl = document.getElementById("player" + i + "color");
		if (colorEl) {
			p.color = colorEl.value.toLowerCase();
		} else {
			// Default color if element not found
			var defaultColors = ['yellow', 'blue', 'red', 'lime', 'green', 'aqua', 'orange', 'purple'];
			p.color = defaultColors[(i - 1) % defaultColors.length];
		}
		...
		if (aiEl && aiEl.value === "0") {
			...
			p.human = true;
		} else if (aiEl && aiEl.value === "1") {
			p.human = false;
			if (typeof AITest !== 'undefined') {
				p.AI = new AITest(p);
			}
		}
	}
